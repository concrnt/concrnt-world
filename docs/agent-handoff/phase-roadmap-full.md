...
## フェーズ2: /keep ルートと Keep 画面

1. `App.tsx` に `/keep` を追加し、`Users / Communities / Messages` タブ付き画面を作る。  
...
2. Keep 時に `LibraryItem.managed` を記録（`watchSubs`, `ack`）。  
3. DoD: Keep 時に Watch/Ack 連動が実行され、`managed` が保存される。  

## フェーズ5: Unkeep の安全な解除（managed 働いているもののみ）

1. Unkeep 時に `managed.watchSubs` があれば unsubscribe、`managed.ack === true` なら `user.UnAck()` を実行。  
